                    ...
                    case 'refresh:party': {
                        ...
                        if(data.displayName && data.meta.schema && data.meta.schema['Default:FrontendEmote_j']) {
                            ...
                            if(emoteItemDef.trim() !== "" && emoteItemDef.trim() !== "None") {
                                ...
                                console.log($(`#${data.id}.member`).children('img')[0]);
                                console.log($(`#${data.id}.member`).children('img'));
...
$(document).ready(async () => {
    ...
    const accounts = await (await fetch(`http://webfort.herokuapp.com/api/accounts`, {
        credentials: 'include',
        headers: {
            'Access-Control-Allow-Origin': '*'
        }
    ...
    for (const account of accounts.accounts) {
        ...
        div.outerHTML = `<div id="${account}" style="overflow: hidden;width: 135px;background: #f3af19;border: 1px solid black;height: 204px;border-radius: 11px;margin: 2px;cursor: pointer;"><div style="background: #f3af19;border: 1px solid white;border-radius: 400px;width: 100px;height: 100px;overflow: hidden;position: relative;left: 17px;top: 10px;"><img src="https://fortnite-api.com/images/cosmetics/br/${cids[cids.length * Math.random() | 0]}/icon.png" style="width: 100px;"></div><div style="font-size: 42px;position: relative;top: 28px;">${account}</div></div>`;
        $(`#${account}`).hover(
            () => {
